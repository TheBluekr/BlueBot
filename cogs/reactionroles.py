...
__author__ = "makupi"
__cogname__ = "bluebot.cogs.reactionroles"
logger = logging.getLogger(__cogname__)

try:
    with open(f"{os.getcwd()}/settings/reactionroles.roles.json") as file:
        reaction_roles_data = json.load(file)
except (FileNotFoundError, json.JSONDecodeError) as ex:
    with open(f"{os.getcwd()}/settings/reactionroles.roles.json", "w") as file:
        json.dump({}, file)
...
class ReactionRoles(commands.Cog):
    ...
    async def on_ready(self):
        self.logger.info(f"ReactionRoles ready.")
    ...
    async def on_raw_reaction_add(self, payload: discord.RawReactionActionEvent):
        ...
        if role is not None and user is not None:
            ...
            self.logger.info(f"Added {role.name} to user {user.name}")
    ...
    async def on_raw_reaction_remove(self, payload: discord.RawReactionActionEvent):
        ...
        if role is not None and user is not None:
            ...
            self.logger.info(f"Removed {role.name} to user {user.name}")
